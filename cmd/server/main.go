...
import (
	...
	"github.com/marketing-digest/auth-service/internal/infrastructure/database"
	"github.com/marketing-digest/auth-service/internal/server"
)

func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "auth fatal: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	store := user.NewGORMStore(db)
	svc := user.NewService(store)
	srv := server.New(svc, log, cfg.GRPCPort)
	srv.SetReady(true)
