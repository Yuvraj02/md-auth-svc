      ...
      steps {
        ...
        script {
          ...
          if (!fileExists('go.mod') || !fileExists('Dockerfile')) {
            error('Expected md-auth-svc repo root (go.mod + Dockerfile). Check Jenkins SCM URL.')
          }
        }
      }
    }
...
EOF
  ...
  post {
    failure {
      echo 'auth-service failed before or during migrate; helm-values was not bumped.'
    }
    success {
