...
// Cluster must already have Secret auth-service-db-url (key: url) in ns marketing-digest.
...
pipeline {
  ...
  environment {
    ...
    ECR_NAME           = 'auth'
    HELM_VALUES_PATH   = 'auth/prod/values.yaml'
    MIGRATE_JOB_TMPL   = 'backend/services/auth-service/deployments/ci-migrate-job.yaml'
  ...
  stages {
    ...
    stage('Deps') {
      steps {
        sh 'go version'
        sh 'cd backend/services/auth-service && make tidy'
      }
    }

    stage('Lint') {
      steps {
        sh 'cd backend/services/auth-service && make lint'
    ...
    stage('Unit Tests') {
      steps {
        sh 'cd backend/services/auth-service && make test'
      }
    }
      ...
      steps {
        withCredentials([usernamePassword(
          ...
          sh '''
            ...
            # Update only the image.tag line (two-space indent under image:)
            sed -i -E "s/^(  tag: ).*/\\1\\"${IMAGE_TAG}\\"/" "${HELM_VALUES_PATH}"
            # Keep repository aligned with the image we just pushed
            ...
            if git diff --cached --quiet; then
              ...
              git commit -m "chore(auth): bump image.tag to ${IMAGE_TAG}"
  ...
  post {
    failure {
      echo 'auth-service failed. helm-values is only bumped after the migration Job succeeds, so Argo CD will not roll out a broken migration.'
    }
    success {
